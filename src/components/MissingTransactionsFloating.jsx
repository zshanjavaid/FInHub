...
const MissingTransactionsFloating = ({
  ...
  return (
    <>
      <Rise className="fixed bottom-6 right-6 z-40">
        <Press
          type="button"
          onClick={onOpen}
          className="flex items-center gap-3 rounded-xl sm:rounded-2xl border border-rose-200/80 bg-white px-4 py-3 shadow-card hover:bg-slate-50 transition-colors group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30"
        ...
        >
          <span className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-rose-100 text-rose-700">
            {badgeCount > 0 ? (
              <span className="fh-pulse-ring absolute inset-0 rounded-xl border border-rose-300/60" aria-hidden />
            ) : null}
            <span className={`relative ${badgeCount > 0 ? 'fh-icon-wiggle' : ''}`} aria-hidden>
              <FiAlertTriangle className="w-5 h-5" />
            </span>
          </span>
          <div className="text-left min-w-0">
            <p className="text-sm font-bold text-slate-800 leading-none">Missing</p>
            <p className="text-xs text-slate-600 mt-1 leading-none">View details</p>
          </div>
          {badgeCount > 0 ? (
